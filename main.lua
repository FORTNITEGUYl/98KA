...
local isnetworkowner = identifyexecutor
		and table.find({ 'AWP', 'Nihon', 'Synapse Z', 'Delta', 'Volt', 'Potassium' }, ({ identifyexecutor() })[1])
		and isnetworkowner
	or function()
		return true
	end
...
local TrapDisabler
local AntiFallPart
local bedwars, remotes, sides, oldinvrender, oldSwing = setmetatable({}, {}), nil, {}, nil, nil
...
local function getNearGround(range)
	...
	local blocks = getBlocksInPoints(
		bedwars.BlockController:getBlockPosition(localPosition - range),
		bedwars.BlockController:getBlockPosition(localPosition + range)
	)
	...
	entitylib.start = function()
		...
		if entitylib.Running then
			...
			table.insert(
				...
				collectionService:GetInstanceAddedSignal('entity'):Connect(customEntity)
			)
			table.insert(
				entitylib.Connections,
				collectionService:GetInstanceRemovedSignal('entity'):Connect(function(ent)
					...
					if plr then
						...
						if anim then
							pcall(function()
								...
								table.insert(
									entity.Connections,
									hum.Animator.AnimationPlayed:Connect(function(playedanim)
...
run(function()
	if not canDebug then
		...
		vape.Libraries.cheatenginelib = loadstring(downloadFile('catrewrite/games/bedwars/engine.luau'))(
			vape,
			vapeEvents,
			entitylib,
			store,
...
run(function()
	...
	bedwars = setmetatable({
		...
		CooldownController = Flamework.resolveDependency(
			'@easy-games/game-core:client/controllers/cooldown/cooldown-controller@CooldownController'
		),
		NotificationController = Flamework.resolveDependency(
		...
		BedwarsKitSkin = canDebug and debug.getupvalue(
			require(replicatedStorage.TS.games.bedwars['kit-skin']['bedwars-kit-skin-meta']).getKitSkinMetadata,
			1
		) or {},
		...
		BlockSelector = require(
			replicatedStorage.rbxts_include.node_modules['@easy-games']['block-engine'].out.client.select['block-selector']
		).BlockSelector,
		BowConstantsTable = canDebug and debug.getupvalue(Knit.Controllers.ProjectileController.enableBeam, 8)
			or { RelX = 0, RelY = 0, RelZ = 0 },
		...
		DefaultKillEffect = require(
			lplr.PlayerScripts.TS.controllers.global.locker['kill-effect'].effects['default-kill-effect']
		),
		...
		KillFeedController = Flamework.resolveDependency(
		...
		PartyController = Flamework.resolveDependency(
		...
		TeamUpgradeMeta = canDebug
				and debug.getupvalue(
					require(replicatedStorage.TS.games.bedwars['team-upgrade']['team-upgrade-meta']).getTeamUpgradeMetaForQueue,
					7
				)
			or require(replicatedStorage.TS.games.bedwars['team-upgrade']['team-upgrade-meta']).OGTeamUpgrades,
		...
		AdetundeUpgradeMeta = require(
			replicatedStorage.TS.games.bedwars.items['frosty-hammer']['frosty-hammer-upgrades']
		).FrostyHammerUpgradeMeta, --debug.getupvalue(require(replicatedStorage.TS.games.bedwars.items['frosty-hammer']['frosty-hammer-upgrades']).getFrostyHammerUpgradeMeta, 1),
	...
	local remoteNames = {
		...
		ConsumeBattery = canDebug and getproto(Knit.Controllers.BatteryController.onKitLocalActivated, 1)
			or function() end,
		...
		ConsumeTreeOrb = canDebug and getproto(Knit.Controllers.EldertreeController.createTreeOrbInteraction, 1)
			or function() end,
		DepositPinata = canDebug and getproto(getproto(Knit.Controllers.PiggyBankController.KnitStart, 2), 5)
			or function() end,
		DragonBreath = canDebug and getproto(Knit.Controllers.VoidDragonController.onKitLocalActivated, 5)
			or function() end,
		...
		EquipItem = canDebug and getproto(
			require(replicatedStorage.TS.entity.entities['inventory-entity']).InventoryEntity.equipItem,
			4
		) or function() end,
		FireProjectile = canDebug
				and debug.getupvalue(Knit.Controllers.ProjectileController.launchProjectileWithValues, 2)
			or function() end,
		...
		HannahKill = canDebug and getproto(Knit.Controllers.HannahController.registerExecuteInteractions, 1)
			or function() end,
		HarvestCrop = canDebug and getproto(getproto(Knit.Controllers.CropController.KnitStart, 4), 1)
			or function() end,
		KaliyahPunch = canDebug and getproto(Knit.Controllers.DragonSlayerController.onKitLocalActivated, 1)
			or function() end,
		MageSelect = canDebug and getproto(Knit.Controllers.MageController.registerTomeInteraction, 1)
			or function() end,
		...
		PickupMetal = canDebug and getproto(Knit.Controllers.HiddenMetalController.onKitLocalActivated, 4)
			or function() end,
		ReportPlayer = canDebug
				and require(lplr.PlayerScripts.TS.controllers.global.report['report-controller']).default.reportPlayer
			or function() end,
		...
		if remoteName == remotes.AttackEntity then
			return {
				...
				SendToServer = function(_, attackTable, ...)
					...
					if
						Reach and Reach.Enabled
						or HitBoxes and HitBoxes.Enabled
						or getgenv().Backtrack and getgenv().Backtrack.Enabled
						or getgenv().FakeLag and getgenv().FakeLag.Enabled
					then
						attackTable.validate.raycast = attackTable.validate.raycast or {}
						attackTable.validate.selfPosition.value += CFrame.lookAt(selfpos, targetpos).LookVector * math.max(
							(selfpos - targetpos).Magnitude - 14.399,
							0
						)
					end
		...
		if obj and obj.Name == 'bed' then
			local suc, res = pcall(function()
				for _, plr in playersService:GetPlayers() do
					if
						obj:GetAttribute('Team' .. (plr:GetAttribute('Team') or 0) .. 'NoBreak')
						and not select(2, whitelist:get(plr))
					then
						return false
					end
				end
	...
	local function getBlockHealth(block, blockpos)
		...
		return (
			blockdata and (blockdata:GetAttribute('1') or blockdata:GetAttribute('Health'))
			or block:GetAttribute('Health')
		)
	end
	...
	calculatePath = function(target, blockpos, findmag, angle, closestPosition)
		if not closestPosition and cache[blockpos] and cache[blockpos][4] > tick() then
		...
		for _ = 1, 10000 do
			...
			for _, side in sides do
				...
				local plrangle = math.acos(
					(gameCamera.CFrame.LookVector * Vector3.new(1, 0, 1)):Dot(
						((block.Position - localPosition) * Vector3.new(1, 0, 1)).Unit
					)
				)
				if plrangle > (math.rad(angle) / 2) then
					continue
				end
		...
		local pos, cost, closestCost = nil, math.huge, math.huge
		for node in air do
			local nodecost = distances[node]
			if closestPosition then
				local nodeclosest = (closestPosition - node).Magnitude
				if nodeclosest < closestCost or (nodeclosest == closestCost and nodecost < cost) then
					pos, cost, closestCost = node, nodecost, nodeclosest
				end
			elseif nodecost < cost then
				pos, cost = node, nodecost
			end
		end
	...
	bedwars.breakBlock = function(block, effects, anim, customHealthbar, instant, legit, sorting, angle, closest)
		...
		local mag = 9e9
		local closestPosition = typeof(closest) == 'Vector3' and closest or nil
		if closest then
			closestPosition = getMousePosition() or entitylib.character.RootPart.Position
		end
		local findmag = breakmethods[sorting] or breakmethods.Health
		if closestPosition then
			findmag = function(part)
				return (closestPosition - part.Position).Magnitude
			end
		end
		...
		if not canDebug then
			...
			for _, v in positions do
				local dpos, dcost, dpath = calculatePath(block, v * 3, findmag, angle, closestPosition)
				local dmag = dpos and ((closestPosition or entitylib.character.RootPart.Position) - dpos).Magnitude

				if dpos then
					if
						(closestPosition and (dmag < mag or (dmag == mag and dcost < cost)))
						or (not closestPosition and (dcost < cost or (dcost == cost and dmag < mag)))
					then
		...
		if pos then
			...
			if (workspace:GetServerTimeNow() - bedwars.SwordController.lastAttack) > 0.4 then
				local breaktype = dblock.Name == 'gumdrop_bounce_pad' and 'stone'
					or bedwars.ItemMeta[dblock.Name].block.breakType
				local tool = store.tools[breaktype]
				if tool then
					if legit then
			...
			bedwars.ClientDamageBlock
				...
				:andThen(function(result)
					if result then
						...
						if effects then
							local blockdmg = (
								blockhealthbar.blockHealth
								- (result == 'destroyed' and 0 or getBlockHealth(dblock, dpos))
							)
						...
						if anim then
							local animation = bedwars.AnimationUtil:playAnimation(
								lplr,
								bedwars.BlockController:getAnimationController():getAssetId(1)
							)
			...
			if newinv.inventory.hand ~= oldinv.inventory.hand then
				...
				if currentHand then
					...
					toolType = handData.sword and 'sword'
						...
						or currentHand.itemType:find('bow') and 'bow'
				end
	...
	vape:Clean(workspace.ChildAdded:Connect(function(projectile)
		task.delay(0, function()
			if
				projectile
				and projectile.Parent
				and entitylib.isAlive
				and projectile:GetAttribute('ProjectileShooter') == lplr.UserId
			then
	...
	task.spawn(function()
		...
		repeat
			if entitylib.isAlive then
				entitylib.character.AirTime = workspace:Raycast(
					(store.rootpart or entitylib.character.RootPart).Position,
					Vector3.new(0, -4.5, 0),
					rayParams
				) and tick() or entitylib.character.AirTime
			end
...
run(function()
    ...
    local aimfuncs = {
    	...
    	Adaptive = function(localcframe, ent, fps)
    		...
    		local speed = (AimSpeed.Value * 0.1 * prog)
    			+ (1 - prog)
    			+ (
    				StrafeIncrease.Enabled
    					and (inputService:IsKeyDown(Enum.KeyCode.A) or inputService:IsKeyDown(Enum.KeyCode.D))
    ...
    local function GetTarget()
    	if lasttarget then
    		...
    		if
    			not lasttarget
    			or not lasttarget.RootPart
    			or not lasttarget.Humanoid
    			or not lasttarget.Humanoid.Health
    			or lasttarget.Humanoid.Health <= 0
    		then
    			return false
    		end
    		...
    		if
    			Targets.Walls.Enabled
    			and entitylib.Wallcheck(localPosition, lasttarget.RootPart.Position, Targets.Walls.Enabled)
    		then
    			return false
    		end
    		return lasttarget
    ...
    local function getAttackData()
    	...
    	if
    		...
    		and (tick() - bedwars.SwordController.lastSwing) > 0.15
    	then
    		return false
    	end
    	...
    	then
    		local ent = GetTarget()
    			or KillauraTarget.Enabled and store.KillauraTarget
...
run(function()
    ...
    local function getAttackData()
    	...
    	if
    		not store.hand.tool
    		or not bedwars.ItemMeta[store.hand.tool.Name].projectileSource and store.hand.toolType ~= 'bow'
    	then
    		return false
    	end
    	if
    		Blacklist.Enabled
    		and table.find(Projectiles.ListEnabled, store.hand.tool.Name == 'glue_trap' and 'gloop' or store.hand.tool.Name)
    	then
    		return false
    	end
    			...
    			launchHook = bedwars.ProjectileLaunchHook:Add('BowAssist', 10, function(nextLaunch, ...)
    				...
    				if projmeta and tick() - update < 0.1 and lastent and lastent.RootPart then
    					...
    					local calc = prediction.SolveTrajectory(
    						...
    						(meta.launchVelocity or 100) * (1 - lplr:GetNetworkPing()),
    						gravity,
    						lastent.RootPart.Position,
    						lastent.RootPart.Velocity,
    						workspace.Gravity,
    			...
    			BowAssist:Clean(runService.PostSimulation:Connect(function(dt)
    				...
    				if ent then
    					...
    					if tick() - lastpredicted < 0.1 then
    						targetinfo.Targets[ent] = tick() + 1
    						local cframe, speed = findAim(
    							gameCamera.CFrame,
    							predicted or ent.RootPart.Position,
    							dt,
    							found,
    							multi + ((entitylib.character.RootPart.Position.Y - ent.RootPart.Position.Y) / 7)
    						)
    	...
    	Function = function(callback)
    		if callback then
    			...
    			bedwars.KnockbackUtil.applyKnockback = function(root, mass, dir, knockback, ...)
    				...
    				local check = not TargetCheck.Enabled
    					or entitylib.EntityPosition({
    						Range = 50,
    						Part = 'RootPart',
    						Players = true,
    	...
    	Function = function(callback)
    		if callback then
    			...
    			bedwars.KnockbackUtil.applyKnockback = function(root, mass, dir, knockback, ...)
    				...
    				local chosen = Mode.Value == 'Random' and ({ 'Left', 'Right', 'Pull' })[rand:NextInteger(1, 3)]
    					or Mode.Value
    				local rdir = chosen == 'Pull' and -velocity
    					or table.find({ 'Left', 'Right' }, chosen) and rotateY(velocity, chosen == 'Left' and 90 or 90)
    					or velocity
    				return old(
    					root,
    					mass,
    					Vector3.new(root.Position.X - rdir.X * 100, dir.Y, root.Position.Z - rdir.Z * 100),
    					knockback,
    					...
    				)
    			end
    							...
    							if top then
    								...
    								connection = runService.PreSimulation:Connect(function()
    									...
    									if entitylib.isAlive and lplr:GetAttribute('LastTeleported') == lastTeleport then
    										local delta = (
    											(top - entitylib.character.RootPart.Position) * Vector3.new(1, 0, 1)
    										)
    										...
    										if ray then
    											for _ = 1, 10 do
    												local dpos = roundPos(ray.Position + ray.Normal * 1.5)
    													+ Vector3.new(0, 3, 0)
    										...
    										if not frictionTable.Speed then
    											root.AssemblyLinearVelocity = (AntiFallDirection * getSpeed())
...
run(function()
    ...
    local function getPlayerClawLevel()
    	...
    	if handItem and handItem.Value then
    		...
    		if itemType == 'summoner_claw_1' then
    			return 1
    		end
    		if itemType == 'summoner_claw_2' then
    			return 2
    		end
    		if itemType == 'summoner_claw_3' then
    			return 3
    		end
    		if itemType == 'summoner_claw_4' then
    			return 4
    		end
    	end
    ...
    local function isUsingSummonerAbility()
    	local suc, res = pcall(function()
    		return bedwars.SummonerKitController:isPlayerCastingSpell(lplr)
    	end)
    	...
    	Function = function(callback)
    		if callback then
    			...
    			repeat
    				...
    				local abilityActive = isUsingSummonerAbility()
    				for _, v in workspace:QueryDescendants('#Summoner_SummonCircle') do
    					local pivot = v:FindFirstChild('Pivot')
    					if
    						pivot
    						and math.floor(pivot.Position.X) == math.floor(entitylib.character.RootPart.Position.X)
    						and math.floor(pivot.Position.Z) == math.floor(entitylib.character.RootPart.Position.Z)
    					then
    						abilityActive = true
    						break
    					end
    				...
    				if plr then
    					...
    					if (currentTime - lastAttackTime) >= attackCooldown then
    						if (abilityActive or isChargingAbility or usedAbility) and not SwingDuringAbility.Enabled then
    							task.wait(0.05)
    							continue
    						end
    						...
    						if ShowAnimation.Enabled then
    							task.spawn(pcall, function()
    								...
    								if
    									bedwars.KnightClient and bedwars.KnightClient.Controllers.SummonerKitSkinController
    								then
    									if
    										bedwars.KnightClient.Controllers.SummonerKitSkinController:isPrismaticSkin(lplr)
    									then
    										bedwars.KnightClient.Controllers.SummonerKitSkinController:applyClawRGB(
    											clawModel
    										)
    									end
    								end
    								clawModel.Parent = workspace
    								debrisService:AddItem(clawModel, 0.75)
    								if
    									(gameCamera.CFrame.Position - entitylib.character.RootPart.Position).Magnitude < 1
    								then
    								...
    								local startPos = rootPart.Position
    									+ Unit:Cross(Vector3.new(0, 1, 0)).Unit * -1 * 5
    									+ Unit * 6
    								...
    								if clawModel:FindFirstChild('AnimationController') then
    									...
    									if animator then
    										bedwars.AnimationUtil:playAnimation(
    											animator,
    											bedwars.GameAnimationUtil:getAssetId(
    												bedwars.AnimationType.SUMMONER_CLAW_ATTACK
    											),
    											{
    												looped = false,
    												speed = 1,
    	...
    	Function = function(callback)
    		if callback then
    			...
    			bedwars.BlockBreaker.hitBlock = function(self, ...)
    				...
    				pcall(function()
    					local block, info =
    						nil, self.clientManager:getBlockSelector():getMouseInfo(1, {
    							ray = params,
    						})
    					...
    					if
    						block
    						and (not Blacklist.Enabled or not find(newlist, block.Name))
    						and (not BedCheck.Enabled or block.Name ~= 'bed')
    					then
    		...
    		if callback then
    			...
    			Fly:Clean(runService.PreSimulation:Connect(function(dt)
    				if entitylib.isAlive and isnetworkowner(entitylib.character.RootPart) then
    					local flyAllowed = (
    						lplr.Character:GetAttribute('InflatedBalloons')
    						and lplr.Character:GetAttribute('InflatedBalloons') > 0
    					) or store.matchState == 2
    					local mass = (1.5 + (flyAllowed and 6 or 0) * (tick() % 0.4 < 0.2 and -1 or 1))
    						+ ((up + down) * VerticalValue.Value)
...
run(function()
    ...
    local function getAttackData()
    	...
    	if Limit.Enabled then
    		if store.equippedKit == 'summoner' then
    			if not store.hand.tool or not store.hand.tool.Name:find('summoner_claw') then
    				return false
    			end
    		else
    	...
    	if LegitAura.Enabled then
    		if
    			(tick() - (bedwars.SwordController.lastSwing or 0)) > (ChargeTime.Value > 0.25 and ChargeTime.Value or 0.11)
    		then
    			return false
    		end
    	end
    ...
    local function getHitreg(distance)
    	...
    	if DynamicHits.Enabled then
    		local farHitreg = math.min(hitreg, bedwars.CombatConstant.RAYCAST_SWORD_CHARACTER_DISTANCE)
    		local dynamicRange = math.max(AttackRange.Value - farHitreg, 1)
    		local scale = math.clamp((distance - farHitreg) / dynamicRange, 0, 1)
    		hitreg += (farHitreg - hitreg) * scale
    	end
    	return hitreg
    end
    ...
    local function getProjectiles()
    	...
    	for _, item in store.inventory.inventory.items do
    		...
    		if ammo and not projectileBlacklist[ammo] then
    			table.insert(items, {
    				item,
    				ammo,
    ...
    local function shootFunc(item, ammo, projectile, itemMeta, pos, ent, ign, legitswitch)
    	...
    	if calc then
    		...
    		task.spawn(function()
    			...
    			local shootPosition = (CFrame.new(pos, calc) * CFrame.new(
    				Vector3.new(
    			...
    			local res = bedwars.Client:Get(remotes.FireProjectile).instance:InvokeServer(
    				item.tool,
    				ammo,
    				projectile,
    				shootPosition,
    		...
    		if callback then
    				...
    				task.spawn(function()
    					...
    					repeat
    						if Attacking then
    							...
    							for _, v in anims[AnimationMode.Value] do
    								AnimTween = tweenService:Create(
    									...
    									TweenInfo.new(
    										first and (AnimationTween.Enabled and 0.001 or 0.1)
    						...
    						elseif started then
    							...
    							AnimTween = tweenService:Create(
    								gameCamera.Viewmodel.RightHand.RightWrist,
    								TweenInfo.new(AnimationTween.Enabled and 0.001 or 0.3, Enum.EasingStyle.Exponential),
    								{
    									C0 = armC0,
    								}
    							)
    			...
    			Killaura:Clean(runService.PostSimulation:Connect(function()
    				if entitylib.isAlive and RangeVisualiser then
    					...
    					RangeVisualiser.Position = entitylib.character.RootPart.Position
    						- Vector3.new(0, entitylib.character.Humanoid.HipHeight, 0)
    				end
    			end))
    			...
    			repeat
    				...
    				if sword and store.matchState ~= 0 then
    					...
    					if #plrs > 0 then
    						...
    						local selfpos = entitylib.character.RootPart.Position
    						local angleLimit = AngleSlider.Value < 360 and math.cos(math.rad(AngleSlider.Value) / 2) or nil
    						local localfacing
    						if angleLimit then
    							localfacing = (inputService.KeyboardEnabled and gameCamera or entitylib.character.RootPart).CFrame.LookVector * flatVector
    							local localfacingMagnitude = localfacing.Magnitude
    							localfacing = localfacingMagnitude > 0 and localfacing / localfacingMagnitude or flatVector
    						end
    						...
    						for i, v in plrs do
    							...
    							if angleLimit then
    								local flatDelta = delta * flatVector
    								local flatMagnitude = flatDelta.Magnitude
    								if flatMagnitude > 0 and localfacing:Dot(flatDelta / flatMagnitude) < angleLimit then
    									continue
    								end
    							end
    							...
    							table.insert(attacked, {
    								Entity = v,
    								Check = deltaMagnitude > AttackRange.Value and BoxSwingColor or BoxAttackColor,
    							})
    								...
    								if not Swing.Enabled and AnimDelay < tick() and not LegitAura.Enabled then
    									...
    									if canDebug or (not Limit.Enabled or store.hand.toolType == 'sword') then
    										if meta.displayName:find('Summoner Claw') then
    											task.spawn(function()
    												bedwars.SummonerClawController:clawAttack(
    													lplr,
    													entitylib.character.RootPart.Position,
    													gameCamera.CFrame.LookVector,
    													sword.itemType or 'summoner_claw_1'
    												)
    											end)
    										else
    							...
    							if deltaMagnitude > AttackRange.Value then
    								continue
    							end
    							...
    							if Attach.Enabled then
    								local newcf = (
    									...
    									- CFrame.lookAt(actualRoot.Position, selfpos).LookVector
    										* math.max((selfpos - actualRoot.Position).Magnitude - 20.4, 0)
    								)
    							...
    							if actualRoot then
    								local targetpos = actualRoot.Position
    									+ (calculatePosition(selfpos, actualRoot) or Vector3.zero)
    								local targetDelta = (targetpos - selfpos)
    								local targetMagnitude = targetDelta.Magnitude
    								local dir = CFrame.lookAt(selfpos, targetpos).LookVector
    								local hitreg = getHitreg(targetMagnitude)
    								local hitregDelay = SyncHitTime.Enabled and math.max(36 - hitreg, 0) / 36 or 0
    								local pos = selfpos + dir * math.max(targetMagnitude - hitreg, 0)
    								swingStart = tick()
    								bedwars.SwordController.lastAttack = workspace:GetServerTimeNow()
    								store.attackReach = (targetMagnitude * 100) // 1 / 100
    								store.attackReachUpdate = tick() + 1
    								...
    								if meta.displayName:find('Summoner Claw') then
    									...
    									if
    										v.Humanoid.FloorMaterial ~= Enum.Material.Air
    										or math.random(1, 100) < AirChance.Value
    									then
    										if tick() < swingCooldown then
    											continue
    										end
    								...
    								if
    									FastHits.Enabled
    									and (FireRate.Value <= 0 or (tick() - lastShot) >= FireRate.Value)
    									and responded
    									and i <= 1
    									and not entitylib.Wallcheck(
    										entitylib.character.RootPart.Position,
    										targetpos,
    										{ gameCamera, lplr.Character, v }
    									)
    								then
    						...
    						if
    							(tick() - lastSwang) < Killaura.Options['Continue Swinging']:GetRandomValue()
    							and not Swing.Enabled
    							and AnimDelay < tick()
    							and not LegitAura.Enabled
    						then
    							swingStart = tick()
    							AnimDelay = swingStart + math.max(ChargeTime.Value, 0.11)
    							pcall(function(...)
    				...
    				for i, v in Boxes do
    					if BoxData[v] == nil and attacked[i] then
    						tweenService
    							:Create(v, TweenInfo.new(BoxAttackSpeed.Value, Enum.EasingStyle[BoxAttackTween.Value]), {
    								Size = Vector3.new(3, 5, 3),
    							})
    							:Play()
    					elseif BoxData[v] and not attacked[i] then
    						tweenService
    							:Create(v, TweenInfo.new(BoxAttackSpeedEnd.Value, Enum.EasingStyle[BoxAttackTween.Value]), {
    								Size = Vector3.zero,
    							})
    							:Play()
    					end
    ...
    DynamicHits = Killaura:CreateToggle({
    	Name = 'Dynamic Hits',
    	Darker = true,
    	Tooltip = 'Lowers hitreg near max attack range and raises it up close.',
    })
...
run(function()
    ...
    local function launchProjectile(item, pos, proj, speed, dir)
    	...
    	pos = pos - dir * 0.1
    	local shootPosition = (
    		CFrame.lookAlong(pos, Vector3.new(0, -speed, 0))
    		* CFrame.new(
    			Vector3.new(
    				-bedwars.BowConstantsTable.RelX,
    				-bedwars.BowConstantsTable.RelY,
    	...
    	if
    		projectileRemote:InvokeServer(
    			...
    			pos,
    			shootPosition.LookVector * speed,
    ...
    local LongJumpMethods = {
    	cannon = function(_, pos, dir)
    		...
    		task.delay(0, function()
    			...
    			if
    				block
    				and block.Name == 'cannon'
    				and (entitylib.character.RootPart.Position - block.Position).Magnitude < 20
    			then
    				local breaktype = bedwars.ItemMeta[block.Name].block.breakType
    				local tool = store.tools[breaktype]
    				if tool then
    				...
    				local broken = 0.1
    				if
    					bedwars.BlockController:calculateBlockDamage(lplr, { blockPosition = blockpos })
    					< block:GetAttribute('Health')
    				then
    					broken = 0.4
    	...
    	tnt = function(item, pos, dir)
    		pos = pos - Vector3.new(0, (entitylib.character.HipHeight + (entitylib.character.RootPart.Size.Y / 2)) - 3, 0)
    		local rounded = Vector3.new(math.round(pos.X / 3) * 3, math.round(pos.Y / 3) * 3, math.round(pos.Z / 3) * 3)
    		start = Vector3.new(rounded.X, start.Y, rounded.Z)
    			+ (dir * (item.itemType == 'pirate_gunpowder_barrel' and 2.6 or 0.2))
    		bedwars.placeBlock(rounded, item.itemType, false)
    	end,
    	wood_dao = function(item, pos, dir)
    		if
    			(lplr.Character:GetAttribute('CanDashNext') or 0) > workspace:GetServerTimeNow()
    			or not bedwars.AbilityController:canUseAbility('dash')
    		then
    			repeat
    				task.wait()
    			until (lplr.Character:GetAttribute('CanDashNext') or 0) < workspace:GetServerTimeNow()
    					and bedwars.AbilityController:canUseAbility('dash')
    				or not LongJump.Enabled
    		end
    	...
    	Function = function(callback)
    		...
    		if callback then
    			LongJump:Clean(vapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
    				if
    					damageTable.entityInstance == lplr.Character
    					and damageTable.fromEntity == lplr.Character
    					and (not damageTable.knockbackMultiplier or not damageTable.knockbackMultiplier.disabled)
    				then
    					local knockbackBoost = bedwars.KnockbackUtil.calculateKnockbackVelocity(Vector3.one, 1, {
    						vertical = 0,
    						horizontal = (
    							damageTable.knockbackMultiplier and damageTable.knockbackMultiplier.horizontal or 1
    						),
    					}).Magnitude * 1.1

    					if knockbackBoost >= JumpSpeed then
    						local pos = damageTable.fromPosition
    								and Vector3.new(
    									damageTable.fromPosition.X,
    									damageTable.fromPosition.Y,
    									damageTable.fromPosition.Z
    								)
    				...
    				if root and isnetworkowner(root) then
    					if JumpTick > tick() then
    						root.AssemblyLinearVelocity = Direction
    								* (getSpeed() + ((JumpTick - tick()) > 1.1 and JumpSpeed or 0))
    							+ Vector3.new(0, root.AssemblyLinearVelocity.Y, 0)
    						if entitylib.character.Humanoid.FloorMaterial == Enum.Material.Air and not start then
    							root.AssemblyLinearVelocity += Vector3.new(0, dt * (workspace.Gravity - 23), 0)
    						else
    							root.AssemblyLinearVelocity =
    								Vector3.new(root.AssemblyLinearVelocity.X, 15, root.AssemblyLinearVelocity.Z)
    						end
    						start = nil
    					else
    						if start then
    			...
    			if store.hand and LongJumpMethods[store.hand.tool.Name] then
    				task.spawn(
    					LongJumpMethods[store.hand.tool.Name],
    					getItem(store.hand.tool.Name),
    					start,
    					(CameraDir.Enabled and gameCamera or entitylib.character.RootPart).CFrame.LookVector
    				)
    				return
    			end
    	...
    	Items = function(position)
    		...
    		if item then
    			if item.itemType == 'telepearl' then
    				...
    				local shootPosition = (CFrame.new(localPosition, position) * CFrame.new(
    					Vector3.new(
    						-bedwars.BowConstantsTable.RelX,
    						-bedwars.BowConstantsTable.RelY,
    						-bedwars.BowConstantsTable.RelZ
    			...
    			elseif
    				...
    				and (localPosition - Vector3.new(position.X, localPosition.Y, position.Z)).Magnitude <= 200
    			then
    				local root = entitylib.character.RootPart
    		...
    		if callback then
    			...
    			launchHook = bedwars.ProjectileLaunchHook:Add('ProjectileAimbot', 100, function(nextLaunch, ...)
    				...
    				if plr then
    					...
    					if
    						table.find(
    							Blacklist.ListEnabled or {},
    							(
    								(projmeta.projectile == 'glue_trap' or projmeta.projectile == 'glue_projectile')
    									and 'gloop'
    								or projmeta.projectile
    							)
    						)
    					then
    						return nextLaunch(...)
    					...
    					local offsetpos = pos
    						+ (projmeta.projectile == 'owl_projectile' and Vector3.zero or projmeta.fromPositionOffset)
    					local balloons = plr.Character:GetAttribute('InflatedBalloons')
    					local playerGravity = workspace.Gravity

    					if balloons and balloons > 0 then
    						playerGravity = (
    							workspace.Gravity * (1 - (balloons >= 4 and 1.2 or balloons >= 3 and 1 or 0.975))
    						)
    					end
    					...
    					local calc = prediction.SolveTrajectory(
    						...
    						gravity,
    						targetpos,
    						projmeta.projectile == 'telepearl' and Vector3.zero or newv,
    						playerGravity,
    						plr.HipHeight,
    					...
    					if calc then
    						...
    						return {
    							initialVelocity = CFrame.new(newlook.Position, calc).LookVector
    								* (projSpeed * (AutoCharge.Enabled and 1 or projmeta.velocityMultiplier)),
    ...
    ProjectileAura = vape.Categories.Blatant:CreateModule({
    	...
    	Function = function(callback)
    		if callback then
    			repeat
    				if
    					(workspace:GetServerTimeNow() - bedwars.SwordController.lastAttack) > 0.5 and not LongJump.Enabled
    				then
    						...
    						for _, data in getProjectiles() do
    							...
    							if (FireDelays[item.itemType] or 0) < tick() then
    								...
    								local calc = prediction.SolveTrajectory(
    									...
    									projSpeed,
    									gravity,
    									ent.RootPart.Position + (ent.Humanoid.MoveDirection or Vector3.zero),
    									ent.RootPart.Velocity,
    									workspace.Gravity,
    									ent.HipHeight + 2,
    									ent.Jumping and 42.6 or nil,
...
run(function()
    ...
    local function Added(ent)
    	...
    	local format = string.format(
    		Strings[ent],
    		tostring(ent:GetAttribute('Level') or 0),
    		(ent:GetAttribute('Level') or 0) >= 2 and 's' or ''
    	)
    			...
    			GeneratorESP:Clean(runService.PreRender:Connect(function()
    				for ent, nametag in Reference do
    					...
    					if (Updates[ent] or 0) > tick() then
    						nametag.Text = string.format(
    							Strings[ent],
    							`| T{ent:GetAttribute('GeneratorLevel')}`,
    							Cooldown[ent] and ` | {getgenv().getNumber(Cooldown[ent].Text)}s` or ''
    						)
    						local size = getfontsize(
    							removeTags(nametag.Text),
    							nametag.TextSize,
    	...
    	Function = function(callback)
    		if callback then
    			...
    			label.TextColor3 = entitylib.isAlive
    					and Color3.fromHSV(
    						(lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) / 2.8,
    						0.86,
    						1
    					)
    				or Color3.new()
...
run(function()
    ...
    local function Added(ent)
    	...
    	nametag.Text = string.format(
    		Strings[ent],
    		'nan',
    		ent:GetAttribute('Amount') >= 2 and ' x' .. tostring(ent:GetAttribute('Amount')) or ''
    	)
    	nametag.TextColor3 = Color3.new(1, 1, 1)
    	nametag.RichText = true
    	nametag.Parent = Folder
    				...
    				for ent, nametag in Reference do
    					...
    					if Distance.Enabled then
    						local mag = entitylib.isAlive
    								and math.floor((entitylib.character.RootPart.Position - ent.Position).Magnitude)
    							or 0
...
run(function()
    ...
    local function getDraftCard(container)
    	...
    	return container.Name == 'MatchDraftPlayerCard' and container
    		or container:FindFirstChild('MatchDraftPlayerCard', true)
    end
    ...
    local function callback5v5(v, plr)
    	...
    	if player then
    		...
    		tweenService
    			:Create(roact, TweenInfo.new(0.2, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
    ...
    local function setup5v5(DraftApp)
    	...
    	local BodyContainer = Background
    		and Background:FindFirstChild('1')
    		and Background['1']:FindFirstChild('BodyContainer')
    	local hooked = false
    	...
    	Function = function(callback)
    		if callback then
    			...
    			repeat
    				if entitylib.isAlive then
    					if
    						entitylib.character.RootPart.Position.Y < lowestpoint
    						and (lplr.Character:GetAttribute('InflatedBalloons') or 0) < 3
    					then
    	...
    	repeat
    		if entitylib.isAlive then
    			...
    			for _, v in objs do
    				...
    				if
    					part
    					and (part.Position - localPosition).Magnitude
    						<= (not Legit.Enabled and specific and math.huge or range)
    				then
    					func(v)
    				end
    			end
    ...
    local AutoKitFunctions = {
    			...
    			if entitylib.isAlive then
    				...
    				for i, v in bedwars.BatteryEffectsController.liveBatteries do
    					if (v.position - localPosition).Magnitude <= 10 then
    						...
    						if
    							not BatteryInfo
    							or BatteryInfo.activateTime >= workspace:GetServerTimeNow()
    							or BatteryInfo.consumeTime + 0.1 >= workspace:GetServerTimeNow()
    						then
    							continue
    						end
    	...
    	bigman = function()
    		kitCollection('treeOrb', function(v)
    			if
    				bedwars.Client
    					:Get(remotes.ConsumeTreeOrb)
    					:CallServer({ treeOrbSecret = v:GetAttribute('TreeOrbSecret') })
    			then
    				v:Destroy()
    			end
    		end, 12, false)
    	...
    	davey = function()
    		...
    		bedwars.CannonHandController.launchSelf = function(...)
    			...
    			if
    				block:GetAttribute('PlacedByUserId') == lplr.UserId
    				and (block.Position - entitylib.character.RootPart.Position).Magnitude < 30
    			then
    	...
    	farmer_cletus = function()
    		kitCollection('HarvestableCrop', function(v)
    			if
    				bedwars.Client
    					:Get(remotes.HarvestCrop)
    					:CallServer({ position = bedwars.BlockController:getBlockPosition(v.Position) })
    			then
    	...
    	grim_reaper = function()
    		kitCollection(bedwars.GrimReaperController.soulsByPosition, function(v)
    			if
    				entitylib.isAlive
    				and lplr.Character:GetAttribute('Health') <= (lplr.Character:GetAttribute('MaxHealth') / 4)
    				and (not lplr.Character:GetAttribute('GrimReaperChannel'))
    			then
    		...
    		repeat
    			if store.hand.tool and store.hand.tool.Name == 'warlock_staff' then
    				...
    				if plr and plr.Character ~= lastTarget then
    					if
    						not bedwars.Client:Get(remotes.WarlockTarget):CallServer({
...
run(function()
    ...
    local function findNearGround(origin)
    	...
    	do
    		for i = 1, 24 do
    			local ray =
    				workspace:Raycast((origin.Position + (Vector3.yAxis * 3)) + (v * i), Vector3.new(0, -60, 0), rayCheck)
    			if ray then
    				return ray.Position
    			end
    		end
    ...
    AutoPearl = vape.Categories.Utility:CreateModule({
    	...
    	Function = function(callback)
    		if callback then
    			...
    			repeat
    				if
    					entitylib.isAlive and (not Limit.Enabled or store.hand.tool and store.hand.tool.Name == 'telepearl')
    				then
    					local root = entitylib.character.RootPart
    					...
    					if
    						pearl
    						and root.Velocity.Y < -100
    						and not workspace:Raycast(root.Position, Vector3.new(0, -200, 0), rayCheck)
    					then
    						if not check then
    							check = true
    							local ground = findNearGround(root.CFrame + Vector3.new(0, 40, 0))
    								or findNearGround(lasty and lasty + Vector3.new(0, 5, 0) or root.CFrame)
...
run(function()
    ...
    local function joinQueue()
    	if
    		not bedwars.Store:getState().Game.customMatch
    		and bedwars.Store:getState().Party.leader.userId == lplr.UserId
    		and bedwars.Store:getState().Party.queueState == 0
    	then
    	...
    	Function = function(callback)
    		if callback then
    			AutoPlay:Clean(vapeEvents.EntityDeathEvent.Event:Connect(function(deathTable)
    				if
    					deathTable.finalKill
    					and deathTable.entityInstance == lplr.Character
    					and isEveryoneDead()
    					and store.matchState ~= 2
    				then
    					joinQueue()
    				end
    			end))
    ...
    local function shootFunc(ignore)
    	if not inputService.MouseEnabled or ignore then
    		...
    		if proj then
    			...
    			local shootPosition, id =
    				(CFrame.new(selfpos, calc) * CFrame.new(
    					Vector3.new(
    						-bedwars.BowConstantsTable.RelX,
    						-bedwars.BowConstantsTable.RelY,
    						-bedwars.BowConstantsTable.RelZ
    			...
    			bedwars.Client
    				:Get(remotes.FireProjectile)
    				:CallServerAsync(proj.tool, ammo, projectile, shootPosition, selfpos, dir * projSpeed, id, {
    					drawDurationSeconds = 1,
    					shotId = httpService:GenerateGUID(false),
    				}, workspace:GetServerTimeNow() - 0.045)
    							...
    							if tick() > start then
    								for _, data in getProjectiles() do
    									if (FireRate[data[1].itemType] or 0) < tick() then
    										local hotbar, old =
    											getHotbar(data[1].tool), store.hand.tool and getHotbar(store.hand.tool) or 0
    										if hotbar and old and hotbarSwitch(hotbar) then
    											local ignore = vape.Modules['Silent Aura'].Enabled
    												or not inputService.MouseEnabled
    											task.wait(Delay.Value)
    											shootFunc()
    											...
    											task.wait(Delay.Value)
    											FireRate[data[1].itemType] = tick()
    												+ (data[4].fireDelaySec + Rate:GetRandomValue())
    											hotbarSwitch(old)
    		...
    		if callback then
    			AutoToxic:Clean(vapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
    				if Toggles.BedDestroyed.Enabled and bedTable.brokenBedTeam.id == lplr:GetAttribute('Team') then
    					sendMessage(
    						'BedDestroyed',
    						(bedTable.player.DisplayName or bedTable.player.Name),
    						'how dare you >:( | <obj>'
    					)
    				...
    				if deathTable.finalKill then
    					...
    					if killed == lplr then
    						if (not dead) and killer ~= lplr and Toggles.Death.Enabled then
    							...
    							sendMessage(
    								'Death',
    								(killer.DisplayName or killer.Name),
    		...
    		if callback then
    			...
    			bedwars.KnockbackUtil.applyKnockback = function(...)
    				...
    				then
    					...
    					task.delay(
    						GroundDelay:GetRandomValue() / 1000,
    						apply,
    						'vertical',
    						env,
    						root,
    	...
    	Function = function(callback)
    		if callback then
    			...
    			if getItem('guided_missile') and plr then
    				local projectile = bedwars.RuntimeLib.await(
    					bedwars.GuidedProjectileController.fireGuidedProjectile:CallServerAsync('guided_missile')
    				)
    				if projectile then
    					...
    					repeat
    						projectile.model:SetPrimaryPartCFrame(
    							CFrame.lookAlong(plr.RootPart.CFrame.p, gameCamera.CFrame.LookVector)
    						)
    						task.wait(0.1)
    				...
    				if entitylib.isAlive then
    					...
    					for _, v in items do
    						...
    						if (localPosition - v.Position).Magnitude <= Range.Value then
    							if
    								Lower.Enabled
    								and (localPosition.Y - v.Position.Y) < (entitylib.character.HipHeight - 1)
    							then
    								continue
    							end
    							task.spawn(function()
    								bedwars.Client
    									:Get(remotes.PickupItem)
    									:CallServerAsync({
    										itemDrop = v,
    									})
    							...
    							task.spawn(function()
    								for _ = 1, 20 do
    									if plr.RootPart and projectile then
    										if
    											not pcall(function()
    												projectile:SetPrimaryPartCFrame(
    													CFrame.lookAlong(
    														plr.RootPart.Position,
    														gameCamera.CFrame.LookVector
    													)
...
run(function()
    ...
    local function hasPermission(plr)
    	repeat
    		task.wait()
    	until plr:GetAttribute('PlayerConnected')
    ...
    local function staffFunction(plr, checktype)
    	...
    	elseif Mode.Value == 'AutoConfig' then
    		local safe = { 'AutoClicker', 'Reach', 'Sprint', 'Hit Fix', 'Staff Detector' }
    		vape.Save = function() end
    ...
    local function checkJoin(plr, connection)
    	if
    		not plr:GetAttribute('Team')
    		and plr:GetAttribute('Spectator')
    		and not bedwars.Store:getState().Game.customMatch
    	then
    		connection:Disconnect()
    ...
    Mode = StaffDetector:CreateDropdown({
    	Name = 'Mode',
    	List = { 'Uninject', 'Profile', 'Requeue', 'AutoConfig', 'Notify', 'Leave', 'Lobby' },
    	Function = function(val)
...
run(function()
    ...
    AutoSuffocate = vape.Categories.World:CreateModule({
    	...
    	Function = function(callback)
    		if callback then
    			repeat
    				local item = store.hand.toolType == 'block' and store.hand.tool.Name
    					or not LimitItem.Enabled and getWool()
...
run(function()
    ...
    local function switchHotbarItem(block)
    	if
    		block
    		and not block:GetAttribute('NoBreak')
    		and not block:GetAttribute('Team' .. (lplr:GetAttribute('Team') or 0) .. 'NoBreak')
    	then
...
run(function()
    ...
    local function save()
    	if point1 and point2 then
    		...
    		point1 = point1 * 3
    		for i, v in tab do
    			i = bedwars.BlockController:getBlockPosition(
    				CFrame.lookAlong(point1, entitylib.character.RootPart.CFrame.LookVector):PointToObjectSpace(i * 3)
    			) * 3
    ...
    local function load(read)
    	local mouseinfo = bedwars.BlockBreaker.clientManager:getBlockSelector():getMouseInfo(0)
    	if mouseinfo and mouseinfo.target then
    		local position = CFrame.new(mouseinfo.placementPosition * 3)
    			* CFrame.Angles(
    				0,
    				math.rad(
    					math.round(
    						math.deg(
    							math.atan2(
    								-entitylib.character.RootPart.CFrame.LookVector.X,
    								-entitylib.character.RootPart.CFrame.LookVector.Z
    							)
    						) / 45
    					) * 45
    				),
    		...
    		for _, v in read do
    			local blockpos = bedwars.BlockController:getBlockPosition((position * CFrame.new(v[1].x, v[1].y, v[1].z)).p)
    				* 3
    			if parts[blockpos] then
    				continue
    			end
    			local handler =
    				bedwars.BlockController:getHandlerRegistry():getHandler(v[2]:find('wool') and getWool() or v[2])
    			if handler then
    					...
    					for i = 0, 2 do
    						if (store.inventory.inventory.armor[i + 1] ~= 'empty') ~= state and ArmorSwitch.Enabled then
    							bedwars.Store:dispatch({
    								...
    								item = store.inventory.inventory.armor[i + 1] == 'empty' and state and getBestArmor(i)
    									or nil,
    								armorSlot = i,
    							})
    							vapeEvents.InventoryChanged.Event:Wait()
...
run(function()
    ...
    local function canBuy(item, currencytable, amount)
    	...
    	if item.require and item.require.teamUpgrade then
    		if
    			(bedwars.Store:getState().Bedwars.teamUpgrades[item.require.teamUpgrade.upgradeId] or -1)
    			< item.require.teamUpgrade.lowestTierIndex
    		then
    			return false
    		end
    	end
    		...
    		if callback then
    			...
    			repeat
    				...
    				if GUI.Enabled then
    					if
    						not (
    							bedwars.AppController:isAppOpen('BedwarsItemShopApp')
    							or bedwars.AppController:isAppOpen('TeamUpgradeApp')
    						)
    					then
    						npc = nil
    					end
    ...
    for i, v in bedwars.TeamUpgradeMeta do
    	...
    	table.insert(
    		...
    		AutoBuy:CreateToggle({
    			...
    			Function = function(callback)
    				npctick = tick()
    				Functions[5 + toggleCount + (v.name == 'Armor' and 20 or 0)] = callback
    		...
    		for _, entry in list do
    			...
    			if ind then
    				(tab[4] and CustomPost or Custom)[ind] = function(currencytable, shop)
    					...
    					if v then
    						local item = getItem(
    							tab[2] == 'wool_white' and bedwars.Shop.getTeamWool(lplr:GetAttribute('Team')) or tab[2]
    						)
    	...
    	if entitylib.isAlive then
    		if SpeedPotion.Enabled and (not attribute or attribute == 'StatusEffect_speed') then
    			...
    			if speedpotion and (not lplr.Character:GetAttribute('StatusEffect_speed')) then
    				for _ = 1, 4 do
    					if bedwars.Client:Get(remotes.ConsumeItem):CallServer({ item = speedpotion.tool }) then
    						break
    					end
    				end
    			end
    		...
    		if Apple.Enabled and (not attribute or attribute:find('Health')) then
    			if
    				(lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth'))
    				<= (Health.Value / 100)
    			then
    				local apple = getItem('orange')
    					or (not lplr.Character:GetAttribute('StatusEffect_golden_apple') and getItem('golden_apple'))
    					or getItem('apple')
    				...
    				if data.dropData and data.dropData.drops then
    					for _, v in data.dropData.drops do
    						if Show.Enabled then
    							local itemDisplay = bedwars.ItemMeta[v.itemType]
    									and bedwars.ItemMeta[v.itemType].displayName
    								or v.itemType

    							notif(
    								'AutoFish',
    								`You can get {v.amount} {itemDisplay:lower()}{v.amount >= 2 and 's' or ''} on ur next fish`,
    								20,
    								'info'
    							)
    						end
    				...
    				then
    					...
    					if
    						...
    						and not workspace:Raycast(
    							entitylib.character.Head.Position + (ray.Direction * 6),
    							Vector3.new(0, -20, 0)
    						)
    					then
...
run(function()
    ...
    vape.Components.HotbarList = function(optionsettings, children, api)
    	...
    	hotbarlist.BackgroundColor3 = optionsettings.Darker
    			and (children.BackgroundColor3 == color.Dark(uipallet.Main, 0.02) and color.Dark(uipallet.Main, 0.04) or color.Dark(
    				uipallet.Main,
    				0.02
    			))
    		or children.BackgroundColor3
    	hotbarlist.Text = ''
    	...
    	windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
    		...
    		hotbarlist.Size =
    			UDim2.fromOffset(220, math.min(43 + windowlist.AbsoluteContentSize.Y / vape.guiscale.Scale, 603))
    	end)
    	...
    	function optionapi:AddHotbar(data)
    		...
    		hotbar.BackgroundColor3 = table.find(self.Hotbars, hotbardata) == self.Selected
    				and color.Light(uipallet.Main, 0.034)
    			or uipallet.Main
    		hotbar.Text = ''
    		...
    		for i = 1, 9 do
    			...
    			slot.Image = hotbardata.Hotbar[tostring(i)]
    					and bedwars.getIcon({ itemType = hotbardata.Hotbar[tostring(i)] }, true)
    				or ''
    		...
    		hotbar.MouseButton1Click:Connect(function()
    			...
    			if ind == optionapi.Selected then
    				...
    				for i = 1, 9 do
    					optionapi.Window['Slot' .. i].ImageLabel.Image = hotbardata.Hotbar[tostring(i)]
    							and bedwars.getIcon({ itemType = hotbardata.Hotbar[tostring(i)] }, true)
    						or ''
    				end
    			else
    ...
    AutoSteal = vape.Categories.Inventory:CreateModule({
    	...
    	Function = function(call)
    		if call then
    			...
    			local crates, items =
    				collection('team-crate', AutoSteal, function(tab, obj)
    					task.delay(0, function()
    			...
    			repeat
    				if entitylib.isAlive then
    					local localPosition = entitylib.character.RootPart.Position
    					if
    						(tick() - Start) >= Delay.Value
    						and (not GUI.Enabled or bedwars.AppController:isAppOpen('ChestApp'))
    					then
    		...
    		if callback then
    			...
    			bedwars.ClickHold.showProgress = function(self)
    				...
    				local countdowntween = tweenService:Create(
    					self.progressRef:getValue(),
    					TweenInfo.new(self.durationSeconds * (Value.Value / 100), Enum.EasingStyle.Linear),
    					{
...
run(function()
    ...
    local function customHealthbar(self, blockRef, health, maxHealth, changeHealth, block)
    	xpcall(function()
    		...
    		tweenService
    			:Create(self.healthbarProgressRef:getValue(), TweenInfo.new(0.3), {
    ...
    local function attemptBreak(tab, localPosition)
    	...
    	if LimitItem.Enabled and not (heldMeta and heldMeta.breakBlock) then
    		return false
    	end

    	local range = Range.Value
    	local bestBlock, bestMagnitude = nil, range * range
    	local serverTime = workspace:GetServerTimeNow()
    	for _, v in tab do
    		local delta = v.Position - localPosition
    		local magnitude = delta:Dot(delta)
    		if magnitude >= bestMagnitude then continue end
    		if not SelfBreak.Enabled and v:GetAttribute('PlacedByUserId') == lplr.UserId then continue end
    		if (v:GetAttribute('BedShieldEndTime') or 0) > serverTime then continue end
    		if not bedwars.BlockController:isBlockBreakable({ blockPosition = v.Position / 3 }, lplr) then continue end
    		bestBlock, bestMagnitude = v, magnitude
    	end
    		...
    		if callback then
    			...
    			repeat
    				...
    				if entitylib.isAlive then
    					...
    					if attemptBreak(Tesla.Enabled and teslas, localPosition) then
    						continue
    					end
    					if attemptBreak(Bed.Enabled and beds, localPosition) then
    						continue
    					end
    					if attemptBreak(Hive.Enabled and hives, localPosition) then
    						continue
    					end
    			...
    			repeat
    				...
    				if crystal then
    					for i, v in bedwars.AdetundeUtil.getUpgradesFromHammer(lplr) do
    						...
    						local nextUpgrade = AutoAdetunde.Options['Buy ' .. i].Enabled
    								and bedwars.AdetundeUpgradeMeta[i].tiers[v + 1]
    							or nil
    						if nextUpgrade then
    			...
    			repeat
    				if entitylib.isAlive then
    					pcall(function()
    						if
    							Collect.Enabled
    							and (not LimitCollect.Enabled or store.hand.tool and store.hand.tool.Name == 'bee_net')
    						then
    							local localPosition = entitylib.character.RootPart.Position
    						...
    						if Deposit.Enabled and getItem('bee') then
    							...
    							for _, v in hives do
    								...
    								if
    									(v:GetAttribute('Level') or 0) < 10
    									and v:GetAttribute('PlacedByUserId') == lplr.UserId
    									and (localPosition - v.Position).Magnitude <= DepositRange.Value
    								then
    									pcall(function()
    				...
    				if entitylib.isAlive and getItem('hammer') then
    					...
    					for _, v in blocks do
    						if not BedCheck.Enabled or (bed.Position - v.Position).Magnitude <= 30 then
    							...
    							if
    								not table.find(Blacklist.ListEnabled, name) and not v:FindFirstChild('BuilderFortify')
    							then
    	...
    	Function = function(callback)
    		AutoCaitlyn:Clean(vapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
    			if
    				entitylib.isAlive
    				and damageTable.fromEntity == lplr.Character
    				and (entitylib.character.RootPart.Position - damageTable.entityInstance.HumanoidRootPart.Position).Magnitude
    					<= Range.Value
    			then
    				...
    				if not state.Kit.activeContract then
    					for _, v in (state.Kit.availableContracts or {}) do
    						if
    							v.target
    							and v.target.Name == damageTable.entityInstance.Name
    							and (Mode == 'On Hit' or damageTable.entityInstance.Humanoid.Health < MinHP.Value)
    						then
    			...
    			repeat
    				if not Streamer.Enabled and entitylib.isAlive then
    					...
    					for i, v in collectionService:GetTagged('treeOrb') do
    						if
    							tick() > (Delay[v] or 0)
    							and (localPosition - v.Spirit.Position).Magnitude <= Range.Value
    						then
    							if Delay.Value > 0 then
    								task.wait(Delay.Value)
    							end
    		...
    		if call then
    			repeat
    				if entitylib.isAlive then
    					...
    					if
    						tool
    						and (not Limit.Enabled or store.hand.tool and store.hand.tool == tool)
    						and entitylib.EntityPosition({
    							Range = Range.Value,
    							Players = Targets.Players.Enabled,
    							NPCs = Targets.NPCs.Enabled,
    	...
    	Function = function(callback)
    		if callback then
    			...
    			hook = function(...)
    				...
    				local res = { old(...) }
    				local launched = not SuccessfulOnly.Enabled
    					or (
    						controller
    						and controller.lastLaunch
    						and (
    							controller.lastLaunch ~= lastLaunch
    							or workspace:GetServerTimeNow() - controller.lastLaunch < 0.5
    						)
    					)
    			...
    			repeat
    				...
    				if entitylib.isAlive then
    					...
    					for _, v in entitylib.List do
    						if
    							v.Player
    							and (SelfHeal.Enabled or v.Player ~= lplr)
    							and (
    								TeammateHeal.Enabled and v.Player:GetAttribute('Team') == lplr:GetAttribute('Team')
    								or not TeammateHeal.Enabled and SelfHeal.Enabled and v.Player == lplr
    							)
    						then
    							local newmag = (localPosition - v.RootPart.Position).Magnitude
    							if newmag <= mag and v.Health < hp and v.Health < v.MaxHealth then
    		...
    		if call then
    			AutoMetal:Clean(proximityPromptService.PromptShown:Connect(function(prompt)
    				if StreamerMode.Enabled then
    					if
    						prompt.Name == 'hidden-metal-prompt'
    						and (not Limit.Enabled or store.hand.tool and store.hand.tool.Name == 'metal_detector')
    					then
    						task.wait(0.1)
    						prompt:InputHoldBegin()
    					end
    			...
    			repeat
    				if not StreamerMode.Enabled and entitylib.isAlive then
    					...
    					for i, v in collectionService:GetTagged('hidden-metal') do
    						if
    							tick() > (Delay[v] or 0)
    							and (localPosition - v.Part.Position).Magnitude <= Range.Value
    							and (not Limit.Enabled or store.hand.tool and store.hand.tool.Name == 'metal_detector')
    						then
    ...
    local function getSlimes()
    	...
    	for _, v in folder:GetChildren() do
    		...
    		if data and data.Tamer.Value == lplr.UserId then
    			table.insert(
    				slimes,
    				{ Data = data, RootPart = v, Name = ({ v.Name:gsub(`_{lplr.Name}`, ''):gsub('Slime', ' Slime') })[1] }
    			)
    		end
    	end
    	return slimes
    	...
    	Function = function(call)
    		if call then
    			repeat
    				if
    					entitylib.isAlive
    					and (not Limit.Enabled or store.hand.tool and store.hand.tool.Name == 'slime_tamer_flute')
    				then
    									...
    									:andThen(function(suc)
    										if suc then
    											...
    											if Notify.Enabled then
    												notif(
    													'Vape',
    													`Directed {v.Name} to {player.DisplayName} ({player.Name})`,
    													5,
    													'info'
    												)
    											end
    		...
    		if call then
    			AutoNyx:Clean(vapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
    				if
    					...
    					and damageTable.fromEntity
    					and damageTable.fromEntity.Name == lplr.Name
    					and entitylib.EntityPosition({
    						Range = 14.4,
    						Part = 'RootPart',
    						Players = Targets.Players.Enabled,
    					...
    					and bedwars.AbilityController:canUseAbility('midnight')
    				then
    			...
    			repeat
    				if entitylib.isAlive and store.equippedKit == 'airbender' then
    					...
    					local ent = entitylib.EntityPosition({
    						...
    						Range = (
    							UseTornando.Enabled and TonradoRange.Value > Range.Value and TonradoRange.Value
    							or Range.Value
    						),
    						Wallcheck = Targets.Walls.Enabled,
    						Players = Targets.Players.Enabled,
    					...
    					if ent then
    						if
    							(localPosition - ent.RootPart.Position).Magnitude <= Range.Value
    							and bedwars.AbilityController:canUseAbility('airbender_tornado')
    						then
    						...
    						if
    							UseTornando.Enabled
    							and (localPosition - ent.RootPart.Position).Magnitude <= TonradoRange.Value
    							and bedwars.AbilityController:canUseAbility('airbender_moving_tornado')
    						then
    		...
    		if call then
    			repeat
    				...
    				if shopNpc and items then
    					...
    					if
    						(
    							chickenData.currency == 'emerald' and Emerald.Enabled
    							or chickenData.currency == 'iron' and Iron.Enabled
    							or chickenData.currency == 'diamond' and Diamond.Enabled
    						) and chickenData.price >= Amount.Value
    					then
    ...
    local function getDrops(localPosition, ItemDrops)
    	...
    	for i, v in ItemDrops do
    		if v.Name == 'emerald' and Emerald.Enabled or v.Name == 'diamond' and Diamond.Enabled then
    			...
    			if
    				magnitude <= lastmag
    				and not entitylib.Wallcheck(localPosition, v.Position, { gameCamera, lplr.Character, v })
    			then
    						...
    						if (healSpirits + attackSpirits) > 0 then
    							...
    							if drop then
    								...
    								local dir = CFrame.lookAt(
    									localPosition,
    									drop.Position + Vector3.new(0, (localPosition - drop.Position).Magnitude / 5, 0)
    								).LookVector * 100
    								...
    								if Animation.Enabled then
    									bedwars.GameAnimationUtil:playAnimation(
    										lplr.Character,
    										bedwars.AnimationType.WIZARD_BALL_CAST
    									)
    									bedwars.SoundManager:playSound(
    										bedwars.SoundList.SPIRIT_SUMMONER_CHANGE_AFFINITY,
    										{}
    									)
    								end
    ...
    AutoWhisper = vape.Categories.Kits:CreateModule({
    	...
    	Function = function(callback)
    		if callback then
    			...
    			repeat
    				local liftReady = Fly.Enabled
    					and (workspace:GetServerTimeNow() - (lplr:GetAttribute('OwlLiftReadyTime') or 0)) > 0
    				local healReady = Heal.Enabled
    					and (workspace:GetServerTimeNow() - (lplr:GetAttribute('OwlHealReadyTime') or 0)) > 0
    							...
    							if plr then
    								...
    								if
    									...
    									and (
    										Threshold.Value >= 100
    										or (
    												plr.Character:GetAttribute('Health')
    												/ plr.Character:GetAttribute('MaxHealth')
    											)
    											<= (Threshold.Value / 100)
    									)
    								then
...
run(function()
    ...
    local function getAttackData()
    	if Limit.Enabled then
    		...
    		return tool,
    			tool and getHotbar(tool) or nil,
    			tool and (tonumber(tool.Name:sub(#tool.Name, #tool.Name)) or 1) or nil
    	end
    							...
    							if UseStrike.Enabled and bedwars.AbilityController:canUseAbility('LIGHTNING_STRIKE') then
    								bedwars.AbilityController:useAbility('LIGHTNING_STRIKE', newproxy(true), {
    									target = ent.RootPart.Position
    										+ ((ent.Humanoid.MoveDirection or Vector3.zero) * (1 + lplr:GetNetworkPing())),
    								})
    								task.wait(Delay.Value)
    							end
    							...
    							if UseStorm.Enabled and level > 1 then
    								if bedwars.AbilityController:canUseAbility('LIGHTNING_STORM') then
    									bedwars.AbilityController:useAbility('LIGHTNING_STORM', newproxy(true), {
    										target = ent.RootPart.Position
    											+ (
    												(ent.Humanoid.MoveDirection or Vector3.zero)
    												* (1 + lplr:GetNetworkPing())
    											),
    									})
    									task.wait(Delay.Value)
    								end
    						...
    						if Mode.Value == 'Legit' then
    							...
    							repeat
    								gameCamera.CFrame = gameCamera.CFrame:Lerp(
    									CFrame.lookAt(gameCamera.CFrame.p, lookpos),
    									22 * runService.PostSimulation:Wait()
    								)
    								bedwars.Client:Get(remotes.CannonAim):SendToServer({
    									cannonBlockPos = blockpos,
    		...
    		if call then
    			FishermanSpy:Clean(bedwars.Client:Get('FishCaught'):Connect(function(data)
    				if data.dropData and data.dropData.drops and data.catchingPlayer then
    					local text = {}
    					for _, v in data.dropData.drops do
    						local itemDisplay = bedwars.ItemMeta[v.itemType] and bedwars.ItemMeta[v.itemType].displayName
    							or v.itemType
    					...
    					if #text > 0 and (not Teammates.Enabled or lplr.Team ~= data.catchingPlayer.Team) then
    						notif(
    							'FishermanSpy',
    							`{data.catchingPlayer.Name} caught {table.concat(text, ', ')}`,
    							20,
    							'info'
    						)
    					end
    ...
    if canDebug then
    	run(function()
    		...
    		local HotbarOpenInventory =
    			require(lplr.PlayerScripts.TS.controllers.global.hotbar.ui['hotbar-open-inventory']).HotbarOpenInventory
    		local HotbarHealthbar =
    			require(lplr.PlayerScripts.TS.controllers.global.hotbar.ui.healthbar['hotbar-healthbar']).HotbarHealthbar
    		local HotbarApp =
    			getRoactRender(require(lplr.PlayerScripts.TS.controllers.global.hotbar.ui['hotbar-app']).HotbarApp.render)
    		local old, new = {}, {}
    		...
    		Interface:CreateColorSlider({
    			...
    			Function = function(hue, sat, val, opacity)
    				...
    				modifyconstant(
    					debug.getupvalue(HotbarApp, 23).render,
    					51,
    					tonumber(Color3.fromHSV(hue, sat, val):ToHex(), 16)
    				)
    				modifyconstant(
    					debug.getupvalue(HotbarApp, 23).render,
    					58,
    					tonumber(
    						Color3.fromHSV(hue, sat, math.clamp(val > 0.5 and val - 0.2 or val + 0.2, 0, 1)):ToHex(),
    						16
    					)
    				)
    	...
    	Function = function(callback)
    		if callback then
    			...
    			KillEffect:Clean(lplr:GetAttributeChangedSignal('KillEffectType'):Connect(function()
    				lplr:SetAttribute(
    					'KillEffectType',
    					Mode.Value == 'Bedwars' and NameToId[List.Value] or 'Custom' .. Mode.Value
    				)
    			end))
    			lplr:SetAttribute(
    				'KillEffectType',
    		...
    		if callback then
    			...
    			repeat
    				...
    				if beattick < tick() and SongBeats.Enabled and FOV.Enabled then
    					...
    					oldfov = math.min(
    						bedwars.FovController:getFOV() * (bedwars.SprintController.sprinting and 1.1 or 1),
    						120
    					)
    	...
    	Function = function()
    		...
    		for _, entry in List.ListEnabled do
    			...
    			if id and #split > 1 then
    				soundlist[id] = split[2]:find('rbxasset') and split[2]
    					or isfile(split[2]) and assetfunction(split[2])
    					or ''
    			end
    		end
    	end,
    ...
    TexturePacks = vape.Legit:CreateModule({
    	...
    	Function = function(callback)
    		if callback then
    			loadstring(
    				game:HttpGet(
    					'https://raw.githubusercontent.com/MaxlaserTech/TexturePacks/main/' .. Pack.Value .. '.lua'
    				),
    				Pack.Value
    			)()
    		else
    ...
    Viewmodel = vape.Legit:CreateModule({
    	...
    	Function = function(callback)
    		...
    		if callback then
    			...
    			if viewmodel then
    				gameCamera.Viewmodel.RightHand.RightWrist.C1 = oldc1
    					* CFrame.Angles(math.rad(Rots[1].Value), math.rad(Rots[2].Value), math.rad(Rots[3].Value))
    			end
    			lplr.PlayerScripts.TS.controllers.global.viewmodel['viewmodel-controller']:SetAttribute(
